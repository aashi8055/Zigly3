 ...
 * written: only a few destinations this app actually sends people to have a
 * shape worth claiming in full (a grid, a product, the dashboard). Everything
 * else -- `'plain'` -- gets a generic set of lines rather than a guessed
 * layout, because a page's own furniture is too varied to claim honestly, but
 * a bare spinner is still the one thing this cover must never fall back to:
 * it reads as a website's own loading indicator, and it is the one shape here
 * that promises nothing at all about what is coming.
 *
 ...
 * placeholder that never comes down.
 */
export const PAGE_COVER_CAP_MS = 4200;

/**
 * The placeholder shapes wait before appearing.
 *
 * A warmed page is often ready inside a couple of hundred milliseconds, and a
 * shape that flashes on and straight off again is noise -- it makes a fast
 ...
 *   plain   everything else: a content page, checkout. No layout worth
 *           claiming, but still a handful of generic lines rather than
 *           nothing -- a page-shaped placeholder beats an empty ground the
 *           same way it beats one for every other variant.
 */
...
const PageCover = ({
  ...
  useEffect(() => {
    ...
    /*
     * The placeholder shapes fade in behind their own delay, so a page that is
     * ready inside a couple of hundred milliseconds is covered by a quiet
     * ground and nothing else. A shape that appears and vanishes again reads
     * as a stutter, which is the opposite of the job.
     */
    ...
    <Animated.View
      ...
      <Animated.View style={[styles.sheet, {opacity: skeleton}]}>
        {variant === 'home' ? (
          ...
          /*
            'plain': a content page, checkout, the all-collections tile list.
            No layout of theirs is worth claiming, so this is a generic
            paragraph shape rather than a guess at furniture the page may not
            even draw -- staggered widths read as "text is coming" without
            promising an image, a grid or anything more specific than that.
          */
          <>
            <Block pulse={pulse} style={[shapes.lineWide, styles.plainTitle]} />
            <Block pulse={pulse} style={shapes.lineWide} />
            <Block pulse={pulse} style={[shapes.lineWide, styles.plainWide]} />
            <Block pulse={pulse} style={shapes.lineNarrow} />
            <Block pulse={pulse} style={[shapes.lineWide, styles.plainGap]} />
            <Block pulse={pulse} style={shapes.lineWide} />
            <Block pulse={pulse} style={shapes.lineNarrow} />
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  /* Rhythm for the 'plain' variant's generic paragraph, top to bottom. */
  plainTitle: {height: 16, width: '60%', marginBottom: 18},
  plainWide: {width: '96%'},
  plainGap: {marginTop: 12},
});
